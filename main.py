...
auth = BearerAuthProvider(
    jwks_uri=f"{config('STYTCH_DOMAIN')}/.well-known/jwks.json",
    issuer=config("STYTCH_DOMAIN"),
    algorithm="RS256",
    audience=config("STYTCH_PROJECT_ID")
)
...
def oauth_metadata(request: StarletteRequest) -> JSONResponse:
    ...
    return JSONResponse(
        {
            "resource": base_url,
            "authorization_servers": [config("STYTCH_DOMAIN")],
